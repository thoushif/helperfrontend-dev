...
export const Home = () => {
  ...
  const getAllItems = async (itemType, dashboardType) => {
    ...
    const apiUrlDashboard = `${baseUrl}/help/all`;
    console.log("apiUrlDashboard is", apiUrlDashboard);
    await axios
      .get(apiUrlDashboard)
      .then((response) => response.data)
      .then((data) => {
        console.log("data.helps", data.data.helps);
        setDashboardItemList(data.data.helps);
